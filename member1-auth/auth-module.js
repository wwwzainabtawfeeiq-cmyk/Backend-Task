...
async function seedAdminIfMissing() {
  try {
    ...
    await query(
      'INSERT INTO users (role_id, full_name, email, password_hash, is_active) VALUES ($1, $2, $3, $4, true)',
      [adminRole[0].id, 'System Administrator', 'admin@test.com', passwordHash]
    );
...
async function getAuthenticatedUser(req) {
  ...
  try {
    ...
    return {
      ...
      phone: user.phone,
      role: user.role,
      permissions: permissions.map(function (p) { return p.slug; })
    };
  } catch (err) {
    return null;
  }
...
app.post('/api/auth/register', async function (req, res) {
  try {
    ...
    const inserted = await query(
      ...
      [clientRole[0].id, body.full_name, body.email, body.phone || null, passwordHash]
    );
